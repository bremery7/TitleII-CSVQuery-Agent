...
- **Strict Rules:**
  ...
  6. For columns like captions_accuracy that might be stored as text but represent numbers:
     - Use: captions_accuracy IS NOT NULL AND CAST(REPLACE(captions_accuracy, '%', '') AS DOUBLE) < 100
     - This handles both "100" and "100%" formats and filters out NULL values
     - Always check IS NOT NULL first when comparing numeric text columns
...
- "captions accuracy less than 100" → SELECT * FROM table WHERE captions_accuracy IS NOT NULL AND CAST(REPLACE(captions_accuracy, '%', '') AS DOUBLE) < 100
- "entries with accuracy below 50 percent" → SELECT * FROM table WHERE captions_accuracy IS NOT NULL AND CAST(REPLACE(captions_accuracy, '%', '') AS DOUBLE) < 50
